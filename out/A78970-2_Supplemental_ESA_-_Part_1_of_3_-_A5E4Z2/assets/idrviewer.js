/*! IDRViewer - v1.10.2 | Copyright 2019 IDRsolutions */
!(function() {
  "use strict";
  var a,
    b,
    ...
    u = [],
    v = !1,
    w = "continuous_horizontal",
    x = !1;
  k.setup = function(r) {
    r || (r = IDRViewer.config),
      (s = !0),
    ...
    for (
      ...
      (G.id = "page" + v),
        G.setAttribute(
          "style",
          "width: " + g[v - 1][0] + "px; height: " + g[v - 1][1] + "px;"
        ),
        (G.className = "page"),
        a.appendChild(G),
        (p[v] = G),
    ...
    c.setup(l),
      L.addClass(b, "layout-" + c.toString()),
      J.updateZoomToDefault(),
      c.goToPage(l),
      A.setPage(l, !0);
    var M = {
  ...
  var y = function(a, b) {
      ...
      return (
        ...
        (c.hide = function() {
          A.getState(b) === z.LOADED &&
            (A.setState(b, z.HIDDEN), (a.firstChild.style.display = "none"));
        }),
        (c.unload = function() {
          (A.getState(b) !== z.LOADED && A.getState(b) !== z.HIDDEN) ||
            (A.setState(b, z.UNLOADED),
            a.removeChild(a.firstChild),
            k.fire("pageunload", { page: b }));
        }),
        (c.load = function() {
          var c = A.getState(b);
          if (
            (c === z.HIDDEN &&
              (A.setState(b, z.LOADED), (a.firstChild.style.display = "block")),
            c === z.UNLOADED)
          )
            if ((A.setState(b, z.LOADING), e)) {
              var d = function() {
              ...
              h.setAttribute("width", "" + g[b - 1][0]),
                h.setAttribute("height", "" + g[b - 1][1]),
                h.setAttribute("data", t + b + (f ? ".svgz" : ".svg")),
                h.setAttribute("type", "image/svg+xml"),
                h.setAttribute("class", "page-inner"),
                  ...
                  c.open("GET", t + b + ".html", !0),
                    (c.onload = function() {
                      if (c.status >= 200 && c.status < 400) {
                        ...
                        var f = function() {
                            A.setState(b, z.LOADED),
                              this && this.removeEventListener("load", f),
                              k.fire("pageload", { page: b });
                          },
                        ...
                        if ((h && g.addEventListener("load", f), t)) {
                          ...
                          l
                            ? g.setAttribute("data", t + l)
                            : ((l = g.getAttribute("src")),
                              l &&
                                l.indexOf("base64") === -1 &&
                                g.setAttribute("src", t + l));
                        }
                        ...
                        o &&
                          ...
                          v || (document.head.appendChild(o), (v = !0))),
                          L.addClass(e, "page-inner"),
                          a.appendChild(e),
                          h || f();
                      } else j();
                    }),
        ...
        p = function() {
          if ((o[a].load(), i < h))
            for (
              var d = 1;
              d < g / 2 &&
              (q(a - d) && (o[a - d].isVisible() || o[a - d].load()),
              i !== h) &&
              (q(a + d) && (o[a + d].isVisible() || o[a + d].load()), i !== h);
              d++
            );
          for (var k = 1, l = m; j + i > g; )
            a - k > l - a
              ? (o[k].isVisible() && o[k].hide(), k++)
              : (o[l].isVisible() && o[l].hide(), l--);
          for (k = 1, l = m; m - c > f; )
            a - k > l - a
              ? (o[k].isLoaded() && o[k].unload(), k++)
              : (o[l].isLoaded() && o[l].unload(), l--);
          b = setTimeout(p, e);
        },
        q = function(a) {
          return a >= 1 && a <= m;
    ...
    C = (function() {
      ...
      (c.setup = function() {}),
        (c.unload = function() {
          for (var b = 1; b <= m; b++)
            (p[b].style.marginLeft = ""),
              (p[b].style.marginTop = ""),
              L.removeClass(p[b], "current", "prev", "next", "before", "after");
      ...
      var d = function(a) {
        for (var b = 1; b <= m; b++)
          L.removeClass(p[b], "current", "prev", "next", "before", "after"),
            b < a
              ? L.addClass(p[b], "before")
              : b > a && L.addClass(p[b], "after");
        L.addClass(p[a], "current"),
          a - 1 >= 1 && L.addClass(p[a - 1], "prev"),
          a + 1 <= m && L.addClass(p[a + 1], "next");
      };
      return (
        (c.updateLayout = function() {
          var c = J.getZoom(),
            d = Math.floor(g[l - 1][0] * c),
            e = 0,
            f = b.clientWidth - q;
          f > d ? (e = (f - d) / 2) : (f = d);
          var h = Math.floor(g[l - 1][1] * c),
            i = 0,
            j = b.clientHeight - q;
          j > h ? (i = (j - h) / 2) : (j = h),
            (a.style.width = f + "px"),
            (a.style.height = j + "px");
          for (var k = 1; k <= m; k++)
            (p[k].style.marginLeft = e + "px"),
              (p[k].style.marginTop = i + "px");
        }),
    ...
    D = (function() {
      ...
      var e = function(a) {
        ...
        if (
          (L.addClass(p[a], "current"),
          c(a) && L.addClass(p[a + 1], "current"),
          1 == a && (a = 0),
          a + 2 <= m &&
            (L.addClass(p[a + 2], "next"),
            a + 3 <= m && L.addClass(p[a + 3], "next")),
          a - 1 > 0 &&
            (L.addClass(p[a - 1], "prev"),
            a - 2 > 0 && L.addClass(p[a - 2], "prev")),
          a + 4 <= m)
        )
          for (b = a + 4; b <= m; b++) L.addClass(p[b], "after");
      ...
      return (
        (d.updateLayout = function() {
          var d = c(l),
            e = J.getZoom(),
            f = Math.floor(g[l - 1][0] * e),
            h = d ? Math.floor(g[l][0] * e) : f,
            i = 2 * Math.max(f, h),
            j = Math.max(i, b.clientWidth - q),
            k = Math.floor(j / 2),
            n = k,
            o = k;
          x ? (o -= h) : (n -= f);
          var r = Math.floor(g[l - 1][1] * e),
            s = d ? Math.floor(g[l][1] * e) : r,
            t = Math.max(r, s, b.clientHeight - q),
            u = Math.floor((t - (x ? s : r)) / 2),
            v = Math.floor((t - (x ? r : s)) / 2);
          (a.style.width = j + "px"),
            (a.style.height = t + "px"),
            (p[1].style.marginLeft = o + "px"),
            (p[1].style.marginTop = v + "px");
          for (var w = 2; w <= m; w += 2)
            (p[w].style.marginLeft = n + "px"),
              (p[w].style.marginTop = u + "px"),
              w < m &&
                ((p[w + 1].style.marginLeft = o + "px"),
                (p[w + 1].style.marginTop = v + "px"));
        }),
        ...
        (d.getZoomBounds = function() {
          var a = c(l),
            b = Math.floor(g[l - 1][0]),
            d = a ? Math.floor(g[l][0]) : 0,
            e = Math.floor(g[l - 1][1]),
    ...
    E = (function() {
      ...
      (a.setup = function() {
        ...
        for (var a = 0; a < m; a++)
          g[a][0] > c && (c = g[a][0]), g[a][1] > d && (d = g[a][1]);
      }),
        (a.unload = function() {
      ...
      var f = function() {
        ...
        h = function() {
          var a, b;
          if (p[1].getBoundingClientRect().top > 0) B(1);
          else
            for (a = 1; a <= m; a++) {
        ...
        i = function() {
          ...
          var a,
            ...
            f = function(a) {
              return (
                (c = p[a].getBoundingClientRect()), c.bottom > 0 && c.top < d
              );
            };
      ...
      return (
        (c.setup = function() {
          ...
          b.addEventListener("touchstart", function(b) {
            l > 1 && (p[l - 1].style.transition = ""),
              (p[l].style.transition = ""),
              l < m && (p[l + 1].style.transition = ""),
              1 === b.touches.length
                ? ((a = Date.now()),
                  (f = b.touches[0].pageX),
                  (k = f),
            ...
            b.addEventListener("touchmove", function(a) {
              if (
                ...
                (h = c - k + h),
                  l > 1 &&
                    (p[l - 1].style.transform =
                      "translate3D(" + (h - b.clientWidth) + "px, 0, 0)"),
                  (p[l].style.transform = "translate3D(" + h + "px, 0, 0)"),
                  l < m &&
                    (p[l + 1].style.transform =
                      "translate3D(" + (h + b.clientWidth) + "px, 0, 0)"),
                  (k = c),
          ...
          for (var o = 0; o < m; o++)
            g[o][0] > d && (d = g[o][0]), g[o][1] > e && (e = g[o][1]);
        }),
        (c.unload = function() {
          (a.style.width = ""), (a.style.height = "");
        }),
        (c.goToPage = function(a) {
          ...
          if (
            ((p[a].style.transition = "0.5s"),
            (p[a].style.transform = "translate3D(0, 0, 0)"),
            a === l)
          )
            a > 1 &&
              ((p[a - 1].style.transition = "0.5s"),
              (p[a - 1].style.transform = "translate3D(" + -j + "px, 0, 0)")),
              a < m &&
                ((p[a + 1].style.transition = "0.5s"),
                (p[a + 1].style.transform = "translate3D(" + j + "px, 0, 0)"));
          else if ("right" === b) {
            (p[l].style.transition = "0.5s"),
              (p[l].style.transform = "translate3D(" + -j + "px, 0, 0)"),
              l > 1 &&
                ((p[l - 1].style.transition = ""),
                (p[l - 1].style.transform = "translate3D(" + -j + "px, 0, 0)"));
            for (var c = l + 1; c < a; c++)
              (p[c].style.transition = ""),
                (p[c].style.transform = "translate3D(" + -j + "px, 0, 0)");
          } else if ("left" === b) {
            (p[l].style.transition = "0.5s"),
              (p[l].style.transform = "translate3D(" + j + "px, 0, 0)"),
              l < m &&
                ...
                (p[l + 1].style.transform = "translate3D(" + j + "px, 0, 0)"));
            for (var d = l - 1; d > a; d--)
              (p[d].style.transition = ""),
                (p[d].style.transform = "translate3D(" + j + "px, 0, 0)");
          }
          setTimeout(function() {
            A.stopLoading();
          }, 250),
        ...
        (c.updateLayout = function() {
          ...
          for (var k = 0; k < l; k++)
            (p[k + 1].style.transform = "translate3D(" + -j + "px, 0, 0)"),
              (p[k + 1].style.top = i + "px"),
              (p[k + 1].style.left = h + "px"),
              (p[k + 1].style.position = "absolute");
          for (
            p[l].style.transform = "translate3D(0, 0, 0)",
              p[l].style.top = i + "px",
              p[l].style.left = h + "px",
              p[l].style.position = "absolute",
              k = l;
            k < m;
            k++
          )
        ...
        a.getSelection
          ? a.getSelection().empty
            ...
            : a.getSelection().removeAllRanges &&
              a.getSelection().removeAllRanges()
          : a.document.selection && a.document.selection.empty();
      } catch (b) {}
    },
    H = function(b) {
      try {
        ...
        for (var c = a.children, d = 0; d < c.length; d++)
          A.getState(d + 1) === z.LOADED && G(c[d].firstChild.contentDocument);
      } catch (e) {}
    },
    I = (function() {
      var a,
      ...
      (f.setup = function() {
        ...
        (this.currentSelectMode = d),
          this.currentSelectMode == k.SELECT_SELECT
            ? f.enableTextSelection()
            : f.enablePanning();
      }),
        (f.enableTextSelection = function() {
          (this.currentSelectMode = k.SELECT_SELECT),
            L.removeClass(h, "panning"),
      ...
      return (
        (f.enablePanning = function() {
          (this.currentSelectMode = k.SELECT_PAN),
            e ? H(window) : G(window),
            L.addClass(h, "panning"),
  ...
  k.setSelectMode = function(a) {
    if (s) {
      if (d) return;
      a == k.SELECT_SELECT ? I.enableTextSelection() : I.enablePanning(),
        k.fire("selectchange", { type: a });
  ...
  var J = (function() {
    var a,
      ...
      e,
      f = k.ZOOM_AUTO,
      h = [0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 2.5, 3, 3.5, 4],
      i = [
        k.ZOOM_AUTO,
        k.ZOOM_FITPAGE,
        k.ZOOM_FITHEIGHT,
      ...
      s = function(e) {
        ...
        var h = !1,
          i = !1;
        l >= 4 ? ((l = 4), (i = !0)) : l <= 0.25 && ((l = 0.25), (h = !0));
        var n = b.scrollTop / b.scrollHeight;
        c.updateLayout();
        for (var q = c.getVisiblePages(), t = 1; t <= m; t++)
          q.indexOf(t) === -1 && o[t].hide();
        a && d.deleteRule(a);
        var u = r(null, 0, 0, l, !1);
        a = d.insertRule(".page-inner { \n" + u + "\n}", d.cssRules.length);
        for (var w = 0; w < m; w++)
          (p[w + 1].style.width = Math.floor(g[w][0] * l) + "px"),
            (p[w + 1].style.height = Math.floor(g[w][1] * l) + "px");
        (b.scrollTop = b.scrollHeight * n),
          j++,
          j % 2 === 1 && s(),
          k.fire("zoomchange", {
            zoomType: f,
      ...
      t = function() {
        for (var a = l, b = h[h.length - 1], c = 0; c < h.length; c++)
          if (h[c] > a) {
            b = h[c];
            break;
          }
      ...
      u = function() {
        for (var a = l, b = h[0], c = h.length - 1; c >= 0; c--)
          if (h[c] < a) {
            b = h[c];
            break;
          }
      ...
      v = function(a) {
        var d = c.getZoomBounds(),
          e = (b.clientWidth - q) / d.width,
          g = (b.clientHeight - q) / d.height,
          h = parseFloat(a);
        switch (
          (isNaN(h) || ((l = h), (a = k.ZOOM_SPECIFIC)), a || (a = f), a)
        ) {
          case k.ZOOM_AUTO:
            l = c.getAutoZoom(e, g);
            break;
  ...
  (k.zoomIn = function() {
    ...
    (k.setLayout = function(a) {
      s
        ? (c.unload(),
          L.removeClass(b, "layout-" + c.toString()),
          a == k.LAYOUT_PRESENTATION
            ? (c = C)
            : a == k.LAYOUT_MAGAZINE
            ? (c = D)
            : a == k.LAYOUT_CONTINUOUS
            ? (c = E)
            : a == w && (c = F),
          c.setup(l),
          L.addClass(b, "layout-" + c.toString()),
          J.updateZoom(IDRViewer.ZOOM_AUTO),
          c.goToPage(l),
          k.fire("layoutchange", { layout: a }))
        : (j = a);
    }),
  ...
  var L = (function() {
    return {
      ...
      removeClass: function() {
        for (
          var a = arguments[0],
            b = 0 !== a.className.length ? a.className.split(" ") : [],
            c = 1;
          c < arguments.length;
          c++
        ) {
